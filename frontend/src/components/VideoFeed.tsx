...
import { saveVideosToCache, loadVideosFromCache } from "../utils/videoCache";
...
export const VideoCard: FC<VideoCardProps> = ({
  ...
  // ← FIX: proxy HQPorner thumbnails through our backend to avoid 403
  const thumbnailSrc = proxyThumbnail(video.thumbnail, video.type);
      ...
      >
        ...
        >
          ...
          <img
            ...
            // no referrerPolicy needed — request goes to our own backend now
          />
...
export default function VideoFeed({
  ...
  // ← Seed from cache immediately so screen is not blank on first render
  const [feedVideos,   setFeedVideos]   = useState<Video[]>(() => loadVideosFromCache());
  const [top10Videos,  setTop10Videos]  = useState<Video[]>([]);
  const [activeVideo,  setActiveVideo]  = useState<Video | null>(null);
  const [loading,      setLoading]      = useState(false);
  const [hasMore,      setHasMore]      = useState(true);

  // Track whether cache was used so we don't flash empty state
  const cacheUsedRef = useRef(feedVideos.length > 0);
  ...
  // Top 10 — only fetch once; not shown on mobile (hidden via CSS)
  useEffect(() => {
    (async () => {
      try {
    ...
    try {
      ...
      if (newVideos.length === 0) {
        ...
        if (page === 1) {
          ...
          // ← Save first page to cache (up to 50 videos for instant reload)
          saveVideosToCache(newVideos);
          cacheUsedRef.current = false;
        } else {
          setFeedVideos((prev) => {
            const merged = [...prev, ...newVideos];
            // Keep cache updated with the best 50 we've seen
            if (merged.length <= 50) saveVideosToCache(merged);
            return merged;
          });
        }
  ...
  useEffect(() => {
    ...
    // Don't wipe cached videos on first mount — let them show while loading
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 items-start">
      ...
      {/*
        SIDEBAR — Top 10
        Hidden completely on mobile/tablet (< lg breakpoint).
        On mobile the top 10 section would take up too much vertical space,
        so we only render it visually on desktop (lg+).
      */}
      <div className="hidden lg:block w-full lg:w-1/4 xl:w-1/5">
        ...
        <div className="space-y-4">
          {top10Videos.map((video, index) => {
            ...
            // ← Also proxy Top 10 thumbnails
            const thumbSrc = proxyThumbnail(video.thumbnail, video.type);
            return (
              <div
                key={video.id}
